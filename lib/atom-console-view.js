...
export default class AtomConsoleView {
    ...
    sendCommand() {
      ...
      } else if (this.commandHandler.currentTool) { // Current workaround as the command-search errors when returning values – likely a bug in Atom
        if (!this.commandHandler.currentTool.specialOutput) {
            this._console.setText(`${contents}[No Match]`);
            this.commandReset = true;
          } else if (this.commandHandler.currentTool.specialOutput) {
