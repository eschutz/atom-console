...
class ConsoleTextEditorView { // Patch for the deprecation of atom-space-pen-views
  ...
  constructor(options = {}) {
    ...
    if (options['mini']) {this.textEditor.setAttribute('mini', 'true')}
...
export default class AtomConsoleView {
    ...
    sendCommand() {
        ...
        if (this.commandHandler.currentTool) {
            if (this.commandHandler.specialOutput) {
                text = this.commandHandler.currentTool.specialOutput;
                cmdReset = true;
            }

            if (this.commandHandler.currentTool.clearConsoleAfterSubmit) {
                text = '';
                cmdReset = false
            }

            if (this.commandHandler.currentTool.specialOutput) {
                this._console.setText(this.commandHandler.currentTool.specialOutput);
                this.commandReset = true;
            }

        } else if (commandOutput != this.commandHandler.commandLog) {
            text = '';
            cmdReset = false;
            if (!commandOutput) {
                text = `${contents}[No Match]`;
                cmdReset = true;
            } else if (typeof commandOutput == 'string') {
                text = commandOutput;
                cmdReset = true;
            }
        }
