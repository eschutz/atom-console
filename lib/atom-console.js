    ...
    toggle() {
        ...
        if (this.atomConsoleVisible) {
            ...
            if (cursor) { // To avoid spec getting null from cursor.setAttribute – Will be cleaned up in future
                let initialCursorHeight = parseInt(cursor.style.height.replace("px", ""));
                let consoleCursorHeight = initialCursorHeight - initialCursorHeight / 4;
                console.log(initialCursorHeight, consoleCursorHeight);
                cursor.setAttribute('style', cursor.getAttribute('style').replace(`height: ${initialCursorHeight}px`, `height: ${consoleCursorHeight}px`));
            };
