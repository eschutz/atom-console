...
export default class Calculator {
    ...
    calculate(expr) {
        ...
        const calcFunctions = Object.keys(this.wordExpressions); // Array of worded functions
        ...
        const cleanUpOperands = (arr, n) => {
            let i1 = arr[n - 1]; // To ensure the same item is referenced after an item is removed
            let i2 = arr[n + 1];
            remove(arr, i1);
            remove(arr, i2);
            compact(arr);
        ...
        if (!(expr.replace(new RegExp(calcFunctions.join('|')), '').match(/[^0-9+\-\*\^%\/\., \(\)]/))) { // Checks expression is valid
            ...
            calcFunctions.forEach(f => {
              exprOperands = evalFunction(exprOperands, f);
            });
            // Exponents
            exprOperands = evalOperations(exprOperands, '^');
            // Division
            exprOperands = evalOperations(exprOperands, '/');
            // Multiplication
            exprOperands = evalOperations(exprOperands, '*');
