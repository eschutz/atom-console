...
const C = new Compiler({
  name: 'gcc',
  options: '-o',
  extensions: ['c', 'h', 'cc', 'cpp', 'cxx', 'c++', 'h', 'hh', 'hpp', 'hxx', 'h++', 'm', 'mm']
});
...
export default function compile() {
  ...
  exec(compiler.format({inputName: file, outputName: compiledFileName}), {cwd:process.env.HOME, env:process.env, shell:_shell},
    (err, stdout, stderr) => {
      if (err || stderr) {
        atom.notifications.addError(`Compilation of \`${file}\` was unsuccessful:`, {description: `Message: \`${err||stderr}\``, dismissable: true});
        return false;
      } else if (stdout) {
