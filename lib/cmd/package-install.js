      ...
      _this = this
        ...
        getPackage(function(error, stdout, stderr) {
          if (error && error.toString().includes("Not Found")) {
            ...
            atom.notifications.addError(`Installing ${pkg} failed.`, {description:`There was an error when installing the package. This may be an issue with the package, which can be raised on its [package page](atom.io/${pkg}).`, stack:stderr, icon:'zap'});
          } else if (stdout){
            _this.output = `Installing ${pkg}... [Enter to continue]`;
            atom.notifications.addSuccess(`${pkg} package sucessfully installed!`, {description:'Reload atom to try it out!', icon:'smiley'});
          } else {
            _this.output = "Error!"
          };
        });
